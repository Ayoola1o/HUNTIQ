 ...
 */
authRouter.get('/onboarding', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  if (!req.user?.workspaceId || !req.user?.id) {
    res.status(401).json({ error: 'Unauthorized' });
    return;
  }
  const onboarding = await authService.getOnboarding(req.user.workspaceId, req.user.id);
  res.status(200).json({
    success: true,
 ...
 */
authRouter.post('/onboarding', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  await authService.saveOnboarding(req.user.id, req.user.workspaceId, onboardingData);
  res.status(200).json({
    success: true,
...
authRouter.post('/api-keys', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  const newKey = await authService.createApiKey(req.user.id, req.user.workspaceId, name);
  res.status(201).json({
    success: true,
...
authRouter.delete('/api-keys/:id', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  const deleted = await authService.deleteApiKey(req.user.id, id);
  if (!deleted) {
 ...
 */
authRouter.get('/activity', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  if (!req.user?.id) {
    res.status(401).json({ error: 'Unauthorized' });
    return;
  }
  const limit = req.query.limit ? Number(req.query.limit) : 50;
  const logs = await authService.getActivityLogs(req.user.id, limit);
  res.status(200).json({
    success: true,
    data: logs
  });
